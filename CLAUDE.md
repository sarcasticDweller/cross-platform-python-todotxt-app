...
Linting:
```bash
uvx ruff check --fix path/to/file      # ruff is picky about unused imports
```
...
**Known unpinned as of 2026-08-10 (needs a follow-up pin):**
- dev group: `pip>=26.1.2`, `pytest>=8.0`, `pytest-cov>=5.0` — floating ranges rather than exact pins. (`pillow` is no longer on this list — it's pinned exactly at `pillow==9.5.0` in the dev group.)

## Architecture

**Import roots.** `pyproject.toml` sets `pythonpath = ["src/lalonde"]`, so all internal imports are rooted *inside* `lalonde`, not at the package itself — e.g. `from tasks_api.task import create_task`, never `from lalonde.tasks_api...`. `buildozer.spec` sets `source.dir = src/lalonde` for the same reason: Buildozer only bundles what's under that directory, so any module living outside `src/lalonde` won't ship to Android (this bit the project once).
...
**`gui/`** — Kivy layer. `main.py` defines `LalondeApp` directly (the earlier `learning_kivy.py` holdover from initially learning Kivy is gone) and builds a `ScreenManager` holding `MainScreen`, `EditTaskScreen`, and `FallbackScreen` (currently a stub, not yet wired to anything). Each screen is a plain Kivy `Screen` with a matching `.kv` file loaded via `Builder.load_file()` in the same module (`main_screen.py`/`.kv`, `edit_task.py`/`.kv`, `fallback_screen.py`/`.kv`; `tasks_view.py`/`.kv` holds the `TaskList`/`TaskWidget` used inside `MainScreen`). GUI widgets don't call `tasks_api.create_task()` directly — they go through `TaskData`/`data_to_task()` and `TaskManager.add_task()`/`update_task()` (e.g. `EditTaskScreen.on_save()`); there's no intermediate view-model layer yet.

**pytodotxt gotchas worth knowing before touching this code:**
...
- `creation_date`/`completion_date` on `pytodotxt.Task` are expected to be `datetime.date`, not `str` — `create_task()` guards both with an explicit `TypeError` rather than letting a `str` crash deep inside serialization (see resolved ISSUE-015).

## Testing conventions

Tests use `@pytest.mark.xfail(reason="...", strict=True)` as a live bug tracker, not just a skip mechanism. In short: a known bug gets a test encoding the *correct* behavior, marked `xfail(strict=True)` with a reason referencing the relevant `ISSUE-NNN`. When the bug is fixed, that test starts `XPASS`ing, which — because of `strict=True` — shows up as a hard suite failure. That failure means "pull the now-stale `xfail` marker off," not "something regressed." A normal (non-`xfail`) test going red is a real regression with no such nuance.

## Tracking known defects

`docs/issues.md` is a hand-maintained defect log (open bugs + doc-drift), reconciled against the code via the `review-issues` skill/slash command — it is not a feature backlog (see `docs/todo.txt` for that, informal as it currently is). That skill's own scope note is worth preserving: it only investigates and updates the issues file, and deliberately never edits source as part of running it.
